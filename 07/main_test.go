...
func TestRunAmplifiers(t *testing.T) {
	...
	testCases := []struct {
		...
		{desc: "ex02", program: "3,31,3,32,1002,32,10,32,1001,31,-2,31,1007,31,0,33,1002,33,7,33,1,33,31,31,1,32,31,31,4,31,99,0,0,0", input: 0, phases: []int{1, 0, 4, 3, 2}, output: 65210},
	}
	for _, tC := range testCases {
		t.Run(tC.desc, func(t *testing.T) {
...
func TestRun(t *testing.T) {
	...
	for _, tC := range testCases {
		t.Run(tC.desc, func(t *testing.T) {
			result, output := run(tC.program, []int{tC.input})
			if output != tC.output {
				t.Fatalf("expected output [%d] got [%d]", tC.output, output)
			}
